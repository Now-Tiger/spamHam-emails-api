...
model_file = os.path.join(
    os.path.dirname(__file__), "email_phishing_detection.0.1.0.joblib"
)

loaded_model: tuple[Pipeline, list[str]] = joblib.load(model_file)
model, targets = loaded_model
vectorizer_file = os.path.join(os.path.dirname(__file__), "./vectorizer.joblib")
...
class EmailDetectionModel:
    ...
    async def predict(self, input: PredictionInput) -> PredictionOutput:
        ...
        * else run prediction
        """
        if not input.text or len(input.text) < 1:
